    ...
    <div className="min-h-screen bg-[#0f0c09] text-white flex items-center justify-center p-4">
      <div className="w-full max-w-md min-h-[860px] rounded-[2.5rem] border border-white/10 bg-gradient-to-b from-[#1d1712] to-[#070605] shadow-2xl overflow-hidden">
        <header className="p-6 pb-3">
          <div className="flex items-center justify-between">
            ...
            <Button
              ...
              className={`h-12 w-12 rounded-full ${isOn ? "bg-amber-400 text-black" : "bg-white/10 text-white"}`}
            >
              <Power size={22} />
            </Button>
          </div>

          <Card className="mt-6 border-white/10 bg-black/30 rounded-3xl overflow-hidden">
            <CardContent className="p-5">
              <div className="flex items-center justify-between">
                ...
                <motion.div
                  animate={{ scale: isOn ? [1, 1.05, 1] : 1, opacity: isOn ? 1 : 0.35 }}
            ...
            return (
              <button
                ...
                className={`min-w-fit rounded-full px-4 py-3 flex items-center gap-2 text-sm transition ${
                  selected ? "bg-amber-400 text-black" : "bg-white/8 text-white/70 border border-white/10"
                }`}
              >
                <Icon size={16} />
                {tab.label}
        ...
        <main className="p-5 pt-2">
          {activeTab === "lumiere" && (
            <Section title="Réglage de la lumière" subtitle="Adaptez CIRCADIA à votre réveil, votre humeur et votre pièce.">
              <SliderCard
              ...
              />
              <FeatureCard title={temperatureLabel} text="Une lumière plus chaude favorise la détente. Une lumière plus claire aide à se sentir plus éveillé le matin." />
            </Section>
          )}

          {activeTab === "heure" && (
            <Section title="Heure et réveil" subtitle="Programmez un lever de soleil progressif, sans réveil brutal.">
              <InputCard icon={<Clock size={20} />} label="Heure de réveil" value={wakeTime} onChange={setWakeTime} type="time" />
              <SliderCard
                icon={<Sun size={20} />}
              ...
              />
              <InputCard icon={<Moon size={20} />} label="Extinction progressive" value={sleepTime} onChange={setSleepTime} type="time" />
              <FeatureCard title="Votre matin commence avant l'alarme" text={`La lumière augmente progressivement ${sunriseDuration} minutes avant ${wakeTime}.`} />
            </Section>
          )}

          {activeTab === "ambiances" && (
            <Section title="Ambiances" subtitle="Choisissez un mode adapté à chaque moment de la journée.">
              <ModeGrid selected={ambientMode} onSelect={setAmbientMode} />
            </Section>
          )}

          {activeTab === "routine" && (
            <Section title="Routine bien-être" subtitle="Créez une journée plus régulière avec des automatismes simples.">
              <ToggleRow icon={<Bell size={20} />} title="Rappel coucher" text="Notification 30 min avant l'extinction." />
              <ToggleRow icon={<Activity size={20} />} title="Suivi du rythme" text="Conseils selon vos horaires de sommeil." />
              <ToggleRow icon={<Moon size={20} />} title="Mode nuit" text="Lumière très douce après 22h." />
              <FeatureCard title="Objectif" text="Vous aider à vous réveiller plus naturellement et à garder un rythme stable." />
            </Section>
          )}

          {activeTab === "maison" && (
            <Section title="Maison connectée" subtitle="Connectez CIRCADIA à vos assistants et à vos pièces.">
              <ToggleRow icon={<Wifi size={20} />} title="Wi-Fi" text="Réseau Maison_5G connecté." activeDefault />
              <ToggleRow icon={<Smartphone size={20} />} title="Application mobile" text="Contrôle à distance activé." activeDefault />
              <ToggleRow icon={<Volume2 size={20} />} title="Commandes vocales" text="Compatible Alexa et Google Assistant." />
              <DeviceCard />
            </Section>
          )}

          {activeTab === "parametres" && (
            <Section title="Réglages" subtitle="Personnalisez l'expérience client et les paramètres de la lampe.">
        ...
        <footer className="p-5 pt-0">
          <Button className="w-full h-14 rounded-2xl bg-amber-400 hover:bg-amber-300 text-black font-semibold">
...
function SliderCard({ icon, title, value, inputValue, min, max, onChange }) {
  return (
    <Card className="rounded-3xl border-white/10 bg-white/6">
      <CardContent className="p-5">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-2xl bg-amber-400/15 text-amber-300 flex items-center justify-center">{icon}</div>
        ...
        return (
          <button
            ...
            className={`text-left rounded-3xl p-4 border transition ${
              active ? "bg-amber-400 text-black border-amber-300" : "bg-white/6 border-white/10 text-white"
            }`}
          >
            <Icon size={24} />
            <p className="mt-4 font-semibold">{mode.name}</p>
            <p className={`text-xs mt-1 ${active ? "text-black/60" : "text-white/45"}`}>{mode.desc}</p>
          </button>
        );
      })}
    </div>
...
function ToggleRow({ icon, title, text, activeDefault = false }) {
  ...
  return (
    ...
    >
      ...
      <div className={`h-7 w-12 rounded-full p-1 transition ${active ? "bg-amber-400" : "bg-white/15"}`}>
        <div className={`h-5 w-5 rounded-full bg-white transition ${active ? "translate-x-5" : "translate-x-0"}`} />
      </div>
    </button>
  );
}
...
function DeviceCard() {
  return (
    <Card className="rounded-3xl border-white/10 bg-gradient-to-br from-white/10 to-white/4">
      <CardContent className="p-5 flex items-center justify-between">
        <div>
